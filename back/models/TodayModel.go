...
func GetPlan(bid *BidRequest) []byte {
	...
	dat, merr := db.Query(sql) //查询到满足条件的所有的广告计划
	if merr == false {
		//fmt.Println(merr)
	}
	//fmt.Println(dat)
	...
	for _, v := range dat { //遍历当前的广告计划，查询是否满足当前的需求
		...
		Planids <- pid
		CheckPlan(v, bid, planids) //检测当前广告计划是否满足条件，满足条件 ，当满足条件的时候添加当前的计划到任务数组中，查询满足条件的广告创意 查询到
	}
...
func CheckPlan(dat map[string]string, bid *BidRequest, planids chan string) {
	//检查显示查询创意
	//创建通道传递当前的
...
func (bid *BidRequest) Chuangyi(pids []string) []byte {
	...
	if isdat == false {
		//当前的时间限制的
		fmt.Println("mysql not found some message")
	}
		...
		for _, vm := range ads.AdType {
			...
			for _, vv := range dat {
				...
				for _, tv := range ad_type_arr {
					...
					if tv_num == int(knum) { //  满足条件的广告创意
						fmt.Println(vv) //一个广告计划下可能有多个创意满足条件
...
func (bid *BidRequest) BbqRequest(dat []map[string]string) []byte {
	...
	for _, ads := range dat {
		...
		biddata.Cid = strconv.FormatUint(cid, 10) //扩展的广告ID

		modelste := &MaterialMeta{} //广告素材对象

		modelste.AdType = AdType_DUANZI_FEED_LP_GROUP
		...
		if mitem["type"] != "3" {
			...
			var imgsA []string
			dsd := strings.Split(mitem["img_url"], ",")
			for _, v := range dsd {
				imgsA = append(imgsA, fmt.Sprintf("%s%s", domain, v))
			}
			imgbanner.Url = imgsA[0]
	...
	proto.UnmarshalMessageSetJSON(data, &d)
	fmt.Println(d)
	if err != nil {
	}
	return data
